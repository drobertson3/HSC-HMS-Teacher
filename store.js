...
const Store = (() => {
  ...
  async function getSettings(){
    ...
    return lsGet('hscq_settings', {});
  }
  async function saveSettings(patch){
    if (cloud) { await db.collection('meta').doc('settings').set(patch, {merge:true}); return; }
    const cur = lsGet('hscq_settings', {});
    lsSet('hscq_settings', {...cur, ...patch});
  }
...
const BADGES = [
  ...
  {id:'health_master',  emoji:'🫀', name:'Health MVP',    desc:'85%+ accuracy over 25+ Health questions'},
  {id:'training_master',emoji:'🏃', name:'Training MVP',  desc:'85%+ accuracy over 25+ Training questions'},
...
function evaluateBadges(student, attempt, totalQuestions){
  ...
  const checks = {
    ...
    health_master:  () => topicAcc('Health') >= 0.85,
    training_master:() => topicAcc('Training') >= 0.85,
